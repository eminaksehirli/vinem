...
public class ClusterInfo {
	...
	public void init(ActionListener buttonsListener, TableModelListener tableModelListener) {
		// the main panel
	...
	public void updateClusterInfo(Map<Integer, Cluster> clustersMap, Set<Integer> clustersToShow) {
		...
		// if clusters have been deleted, we don't need to keep selected rows in sync
	...
	// returns the cluster ids on the rows selected by the user (returns null if no selection)
	...
	public class ClusterTable extends AbstractTableModel {
		private String[] columnNames = {"Visible",
                                        "Cluster id",
                                        "Size",
                                        "Dims",
                                        "Objects"};
        ...
        public void setRows(Map<Integer, Cluster> clustersMap, Set<Integer> clustersToShow) {
        	...
        	for (int clusterId : clustersMap.keySet()) {
        		...
        		// Size
        		data[row][2] = new Integer(clustersMap.get(clusterId).getObjects().size());
        		
        		// Dims
        		Set<Integer> dims = new HashSet<Integer>(clustersMap.get(clusterId).getDims());
        		data[row][3] = dims;
        		
        		// Objects
        		Set<Integer> objs = new HashSet<Integer>(clustersMap.get(clusterId).getObjects());
        		data[row][4] = objs;
