...
public class CartiModel 
{
	...
	private Map<Integer,Integer> getObjId2LocMap(int dim) {
		...
		Map<Integer,Integer> id2LocMap = new HashMap<Integer,Integer>(objId2LocMaps[dim].size() - filtereds.size());
	...
	private Map<Integer,Integer> getLoc2ObjIdMap(int dim) {
		...
		// account for filtereds
		int putLoc = 0;
		Map<Integer,Integer> loc2IdMap = new HashMap<Integer,Integer>(loc2ObjIdMaps[dim].size() - filtereds.size());
	...
	public int[] getSupports(Set<Integer> objIds) {
		for (int id : objIds) {
			if (filtereds.contains(id)) {
				System.err.println("Getting support for object which has been filtered: " + id);
			}
		}
	...
	public double[] getStandardDeviations(Set<Integer> objIds) {
		for (int id : objIds) {
			if (filtereds.contains(id)) {
				System.err.println("Getting Standard Deviation for object which has been filtered: " + id);
			}
		}
		...
		for (int dimIx = 0; dimIx < dims.size(); dimIx++) {
			...
			for (int id : objIds) {
				standardDeviations[dimIx] += Math.pow((means[dimIx] - origData[id][dimIx].v), 2);
			}
			standardDeviations[dimIx] = Math.sqrt(standardDeviations[dimIx] / objIds.size());
		}
	...
	public int[] getMeasures(Set<Integer> objIds) {
		for (int id : objIds) {
			if (filtereds.contains(id)) {
				System.err.println("Getting Measure for object which has been filtered: " + id);
			}
		}
	...
	// Calculates the median absolute deviation based on locations of the given objects in every dimension
	public int[] getLocsMedAbsDev(Set<Integer> objIds) {
		for (int id : objIds) {
			if (filtereds.contains(id)) {
				System.err.println("Getting median absolute deviation for object which has been filtered: " + id);
			}
		}
	...
	public Set<Integer> getClustersToShowLocs() {
		...
		for (int clusterId : clustersToShow) {
			// only show this cluster in its dimensions
			if (clustersMap.get(clusterId).getDims().contains(orderDim)) {
